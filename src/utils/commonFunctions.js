...
async function generateUserIdByEnyId(eny_id) { 
    data = await runQuery(`SELECT * from faculity_users WHERE eny_id = ?`, [eny_id])
    if(data.length > 0){
...
async function getEmployerIdByEnyId(emp_eny_id) { 
    data = await runQuery(`SELECT * from employer_user WHERE emp_eny_id = ?`, [emp_eny_id])
    if(data.length > 0){
        return data[0].employerID;
    }else{
        return null;
    }
}
...
async function replaceEmployerID(keyArray, emp_eny_id = null , employerID) {    
    const employerKeyArray = [
        "employerID" 
    ] 
    if(employerKeyArray.includes(keyArray)){
        if(!emp_eny_id){
            data = await runQuery(`SELECT emp_eny_id from employer_user WHERE employerID = ?`, [employerID]) 
