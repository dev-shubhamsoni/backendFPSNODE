const { sendError, sendSuccess, getDateFormat, convertToAmPm, generateUserIdByEnyId, replaceFacultyID } = require("../../utils/commonFunctions");
...
exports.applyJob = async (req, res) => {
    try {
        const { facultyID:faculty_id, jobID, date1, date2, expected_joining_date, regToken, device_type, duration_notice_period } = req.body
        if (!faculty_id) {
            return sendError(res, { message: "Please enter your user ID..." });
        } else if (!jobID) {
            return sendError(res, { message: "Please enter your job ID..." });
